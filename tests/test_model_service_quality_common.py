"""Unit tests for model_service_quality/common pure functions."""
...
from luban_meter.benchmarking.model_service_quality.common import (
    metrics,
    parsers,
    prompts,
)
...
def test_extract_humaneval_completion_preserves_base_generation() -> None:
    ...
    assert (
        parsers.extract_humaneval_completion(completion, prompt) == completion
    )
...
def test_extract_humaneval_completion_strips_repeated_prompt_only() -> None:
    prompt = "def f():\n"
    assert (
        parsers.extract_humaneval_completion(
            prompt + "\n    return 1\r\n", prompt
        )
        == "    return 1\n"
    )
...
def test_dataset_loading_and_sampling(tmp_path: Path) -> None:
    ...
    assert [
        record["id"] for record in select_records(records, max_samples=2)
    ] == [
        "1",
        "2",
    ]
...
def test_resolve_data_path_falls_back_to_bundled_package_data(
    tmp_path: Path,
) -> None:
    ...
    # A default YAML relative path resolves to the bundled package data
    # dir.
    bundled = resolve_data_path("data/ceval/val.jsonl")
    assert bundled.is_file()
    assert resolve_data_path(bundled) == bundled

    # A missing relative path keeps its original form so load_records
    # raises.
