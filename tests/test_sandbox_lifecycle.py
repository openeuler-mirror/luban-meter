"""Lifecycle regression tests: ownership, cancellation and cleanup
failures.
"""
...
def make_scope(tmp_path):
    return DockerSandboxScope(
        tmp_path,
        DockerSandboxConfig(
            docker_host="unix:///test/docker.sock", image="test"
        ),
    )
...
def test_scope_removes_only_its_own_containers(tmp_path, monkeypatch):
    ...
    containers = {
        "owned": ("candidate", scope.token),
        "other": ("other", "other-run"),
    }
...
def test_interrupted_create_is_not_reported_clean(tmp_path, monkeypatch):
    ...
    with pytest.raises(DockerSandboxUnavailableError, match="unconfirmed"):
...
def test_remove_failure_is_not_silenced(monkeypatch):
    sandbox = DockerSandbox(
        DockerSandboxConfig("unix:///test/docker.sock", "test")
    )
    ...
    with pytest.raises(
        DockerSandboxUnavailableError, match="daemon unavailable"
    ):
        sandbox.remove_container("candidate")


@pytest.mark.parametrize(
    "error", [ExecutionError("timeout"), KeyboardInterrupt()]
)
def test_host_always_cleans_scope_and_stops_monitor(
    ...
    monkeypatch.setattr(
        "luban_meter.execution.host.DeviceMonitorDaemon", Monitor
    )
    monkeypatch.setattr(
...
@pytest.mark.skipif(
    os.name != "posix", reason="POSIX process-group termination"
)
...
def test_sigterm_unwinds_runner_and_restores_handler(tmp_path):
    ...
    with pytest.raises(ExecutionError, match="interrupted by signal"):
        LocalCommandRunner().run(
            CommandSpec([sys.executable, "-c", child], timeout=5)
        )
...
def test_create_failure_never_starts_candidate(tmp_path, monkeypatch):
    from luban_meter.benchmarking.model_service_quality.humaneval import (
        executor as humaneval_executor,
    )
    from luban_meter.utils.docker_sandbox import REGISTRY_ENV

    executor = humaneval_executor.DockerSandboxExecutor(
        humaneval_executor.SandboxConfig("unix:///test/docker.sock", "test")
    )
    monkeypatch.setenv(REGISTRY_ENV, str(tmp_path))

    def create(**kwargs):
        raise DockerSandboxUnavailableError("interrupted create")

    monkeypatch.setattr(executor.sandbox, "create_container", create)
    monkeypatch.setattr(
        executor.sandbox, "remove_container", lambda name: None
    )
    ...
    with pytest.raises(
        DockerSandboxUnavailableError, match="interrupted create"
    ):
...
@pytest.mark.skipif(
    os.name != "posix"
    or os.environ.get("LUBAN_METER_RUN_DOCKER_TESTS") != "1",
...
def test_real_outer_timeout_reclaims_container(tmp_path):
    ...
    script = (
        ...
        "from pathlib import Path\n"
        "from luban_meter.utils.docker_sandbox import "
        "DockerSandbox, DockerSandboxConfig, REGISTRY_ENV\n"
        "s = DockerSandbox(DockerSandboxConfig("
        f"{config.docker_host!r}, {config.image!r}))\n"
        ...
        "p = subprocess.Popen([*s.client_prefix(), 'start', '--attach', "
        "'--interactive', cid], stdin=subprocess.PIPE, "
        "stdout=subprocess.PIPE, stderr=subprocess.PIPE, "
        "env=s.environment())\n"
        "for _ in range(100):\n"
        "    if s._control(['inspect', '--format', "
        "'{{.State.Running}}', cid]) == 'true':\n"
        "        break\n"
        "    time.sleep(0.05)\n"
        "else:\n"
    ...
    try:
        try:
            ...
            pytest.fail(
                f"helper exited before timeout ({result.returncode}): "
                f"{result.stderr}"
            )
    finally:
        scope.cleanup()
