"""Tests for the wikitext inference benchmark."""
...
from luban_meter.benchmark.inference.wikitext import (
    benchmark as wikitext,
)
...
def test_validate_records_duplicate_id() -> None:
    records = [
        {
            "id": "a", "text": "x", "raw_text": "x",
            "bytes": 1, "words": 1,
        },
        {
            "id": "a", "text": "y", "raw_text": "y",
            "bytes": 1, "words": 1,
        },
    ]
...
def test_process_success() -> None:
    ...
    assert wt["mean_loss"]["value"] == pytest.approx(
        expected_ml
    )
    assert wt["perplexity"]["value"] == pytest.approx(
        expected_ppl
    )
    assert wt["bits_per_byte"]["value"] == pytest.approx(
        expected_bpb
    )
...
def test_registry_discovers_wikitext() -> None:
    ...
    assert "wikitext" in registry.list_benchmarks(
        "inference"
    )
    config = (
        ROOT
        / "src" / "luban_meter" / "benchmark"
        / "inference" / "wikitext" / "wikitext.yaml"
    )
    request = RunRequest(
        run_id="inference-wikitext-test",
        module="inference",
        benchmark="wikitext",
        config=config,
        model_path=None,
