...
ENGINE_RESULT_HANDLER = ROOT / (
    ...
    "/offline_vllm_engine/calculate_metrics.py"
)
ENGINE_BENCHMARK_ENTRY = ROOT / (
    "src/luban_meter/benchmarking/generation_performance"
    "/offline_vllm_engine/collect_raw.py"
)
...
def make_case(
    *, output_tokens: int = 3, rounds: list[dict] | None = None
) -> dict:
    return {
        "input_length": 4,
        "output_length": output_tokens,
        ...
        else [make_round(output_tokens=output_tokens)],
    }
...
class EngineSloConfigTest(unittest.TestCase):
    ...
    def test_absent_config_is_backward_compatible(self) -> None:
        self.assertIsNone(self.collector_module.engine_slo_config({}))
        self.assertIsNone(
            self.collector_module.engine_slo_config({"engine_slo": None})
        )
    ...
    def test_valid_partial_config_is_normalized(self) -> None:
        config = self.collector_module.engine_slo_config(
            {
                "engine_slo": {
    ...
    def test_invalid_configs_are_rejected(self) -> None:
        ...
        for parameters in invalid:
            with (
                ...
                self.collector_module.engine_slo_config(parameters)
    ...
    def test_run_benchmark_preserves_config_in_raw_metadata(self) -> None:
        ...
        with (
            ...
            patch.object(
                self.collector_module, "build_engine", return_value=fake_engine
            ),
            patch.object(
                self.collector_module, "generate_batch", return_value=[record]
            ),
            patch.object(
    ...
    def test_failure_result_uses_offline_measurement_name(self) -> None:
        failure = self.collector_module.failure_result(RuntimeError("failed"))
        self.assertEqual(
...
class EngineGoodputTest(unittest.TestCase):
    ...
    def test_classifies_requests_with_and_semantics(self) -> None:
        ...
        self.assertEqual(
            goodput["measurement_boundary"], "vllm_engine_internal"
        )
        self.assertEqual(
    ...
    def test_decode_only_target_is_not_applicable_for_single_token_case(
        self,
    ) -> None:
        raw_result = make_raw_result(
    ...
    def test_non_finite_raw_metric_is_rejected(self) -> None:
        raw_result = make_raw_result(make_case(), {"internal_ttft_ms": 200})
