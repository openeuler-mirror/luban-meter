...
from luban_meter.benchmarking.model_service_quality.common.parsers import (
    general_postprocess,
    lcsts_postprocess,
)
...
def test_validate_parameters(tmp_path: Path) -> None:
    config = lcsts.validate_parameters(base_parameters(tmp_path))
    ...
    with pytest.raises(ValueError):
        lcsts.validate_parameters({**base_parameters(tmp_path), "few_shot": 4})
...
def test_render_lcsts_prompt_few_shot() -> None:
    ...
    prompt = render_lcsts_prompt(sample, few_shot_samples=few_shot)
...
def test_lcsts_postprocess() -> None:
    assert (
        lcsts_postprocess("1. \u6458\u8981\u5185\u5bb9")
        == "\u6458\u8981\u5185\u5bb9"
    )
    assert lcsts_postprocess("- \u6458\u8981") == "\u6458\u8981"
    assert (
        lcsts_postprocess("\u7b2c\u4e00\u884c\n\u7b2c\u4e8c\u884c")
        == "\u7b2c\u4e00\u884c"
    )
    assert lcsts_postprocess("\u201c\u6458\u8981\u201d") == "\u6458\u8981"
...
def test_general_postprocess() -> None:
    ...
    assert (
        general_postprocess("\u6d4b\u8bd5\u3002\u622a\u65ad")
        == "\u6d4b\u8bd5\u622a\u65ad"
    )
    # ASCII period triggers truncation.
    assert general_postprocess("\u6d4b\u8bd5.\u622a\u65ad") == "\u6d4b\u8bd5"
    # Newline triggers truncation.
    assert general_postprocess("\u6d4b\u8bd5\n\u622a\u65ad") == "\u6d4b\u8bd5"
...
def test_prepare_samples_rejects_bad_content() -> None:
    with pytest.raises(ValueError):
        lcsts.prepare_samples([{"content": "", "abst": "x"}])
    with pytest.raises(ValueError):
        lcsts.prepare_samples([{"content": "x", "abst": ""}])
...
def test_run_benchmark_end_to_end(tmp_path: Path) -> None:
    ...
    with patch(
        "luban_meter.benchmarking.model_service_quality.lcsts.collect_raw.OpenAIClient",
        return_value=mock_client,
    ):
        raw = lcsts.collect_raw_result({"model_name": "test-model"}, params)
...
def test_process_identical_texts() -> None:
    ...
    assert lcsts_m["rouge1"]["value"] == pytest.approx(100.0)
    assert lcsts_m["rouge2"]["value"] == pytest.approx(100.0)
    assert lcsts_m["rougeL"]["value"] == pytest.approx(100.0)
