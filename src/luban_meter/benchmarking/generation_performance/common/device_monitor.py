"""Shared device (GPU/NPU/CPU) detection and monitoring via Prometheus
exporters.

Collects hardware metrics from a Prometheus-compatible exporter
endpoint (e.g. DCGM exporter for NVIDIA GPUs, node_exporter for
CPU/memory).

The exporter URL is provided by the user. If not provided,
...
Provides:
  ...
  - HardwareSnapshot / DeviceSample / CpuSample / DeviceInfo — data classes
"""
...
def _parse_prom_value(
    ...
    for line in text.splitlines():
        ...
        metric_match = re.match(r"^(\w+)(?:\{([^}]*)\})?\s+(\S+)$", line)
        if not metric_match:
            continue
        name, label_text, value_text = (
            metric_match.group(1),
            metric_match.group(2),
            metric_match.group(3),
        )
        if name != metric_name:
            continue
        label_dict: dict[str, str] = {}
        if label_text:
            for label_match in re.finditer(r'(\w+)="([^"]*)"', label_text):
                label_dict[label_match.group(1)] = label_match.group(2)
        if labels and not all(
            label_dict.get(label_name) == sample_value
            for label_name, sample_value in labels.items()
        ):
            continue
        try:
            results.append((label_dict, float(value_text)))
        except ValueError:
            pass  # skip string values like "NVIDIA H20"
    return results


def _parse_prom_str_value(
    text: str, metric_name: str, labels: dict[str, str] | None = None
    ...
    for line in text.splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        metric_match = re.match(r'^(\w+)(?:\{([^}]*)\})?\s+"?([^"]+)"?$', line)
        if not metric_match:
            continue
        name, label_text, value_text = (
            metric_match.group(1),
        ...
        if labels and not all(
            ...
            continue
        results.append((label_dict, value_text))
    return results
...
def print_hardware_info(
    devices: list[DeviceInfo] | None = None, exporter_url: str | None = None
) -> None:
    ...
    if not devices:
        print(
            (
                "[Device Monitor] No supported compute devices "
                "detected at the exporter endpoint."
            )
        )
        return
    ...
    tool = devices[0].tool
    print(
        f"[Device Monitor] Detected {len(devices)} x "
        f"{devices[0].name} ({vendor}) via {tool}"
    )
    for dev in devices:
...
def _sample_gpu_from_exporter(
    metrics_text: str, devices: list[DeviceInfo]
) -> list[DeviceSample]:
    ...
    for dev in devices:
        ...
        mem_used = _parse_prom_value(
            metrics_text, "DCGM_FI_DEV_FB_USED", labels
        )
        if mem_used:
            sample.memory_used_mb = mem_used[0][1]

        mem_total = _parse_prom_value(
            metrics_text, "DCGM_FI_DEV_FB_TOTAL", labels
        )
        if mem_total:
            sample.memory_total_mb = mem_total[0][1]

        power = _parse_prom_value(
            metrics_text, "DCGM_FI_DEV_POWER_USAGE", labels
        )
        if power:
...
def _sample_cpu_from_exporter(metrics_text: str) -> CpuSample:
    ...
    idle_samples = _parse_prom_value(
        metrics_text, "node_cpu_seconds_total", {"mode": "idle"}
    )
    cpu_time_samples = _parse_prom_value(
        metrics_text, "node_cpu_seconds_total"
    )
    if idle_samples and cpu_time_samples:
        idle_total = sum(sample_value for _, sample_value in idle_samples)
        all_total = sum(sample_value for _, sample_value in cpu_time_samples)
        if all_total > 0:
            cpu_sample.utilization_percent = round(
                (1 - idle_total / all_total) * 100, 2
            )

    # Memory: node_memory_MemTotal_bytes /
    # node_memory_MemAvailable_bytes
    mem_total = _parse_prom_simple(metrics_text, "node_memory_MemTotal_bytes")
    mem_avail = _parse_prom_simple(
        metrics_text, "node_memory_MemAvailable_bytes"
    )
    if mem_total is not None:
        cpu_sample.memory_total_mb = round(mem_total / (1024 * 1024), 1)
    if mem_total is not None and mem_avail is not None:
        cpu_sample.memory_used_mb = round(
            (mem_total - mem_avail) / (1024 * 1024), 1
        )
...
def collect_hardware_environment(
    ...
    if devices:
        ...
        env["devices"] = [
            {"index": device.index, "name": device.name} for device in devices
        ]
...
def sample_exporter(
    ...
    snapshot = HardwareSnapshot(
        index=snapshot_index, elapsed_seconds=round(elapsed, 6)
    )
    metrics_text = _fetch_metrics(exporter_url)
    if not metrics_text:
        snapshot.error = {
            "type": "FetchError",
            "message": f"Failed to fetch metrics from {exporter_url}",
        }
        return snapshot
