...
def collect_completion_stream(
    ...
    """Consume an SSE stream and retain only valid, non-empty output
    events.
    """
    ...
    if not event_times:
        raise RuntimeError(
            "stream ended before the first non-empty generated output"
        )
    if usage is None:
        raise RuntimeError(
...
def collect_chat_stream(
    ...
    """Consume an SSE chat completion stream and retain output events.

    Unlike ``collect_completion_stream``, this parses the
    ``choices[0].delta.content`` field used by the
    ``/v1/chat/completions`` endpoint instead of
    ``choices[0].text``.
    """
    event_times: list[float] = []
    generated_parts: list[str] = []
