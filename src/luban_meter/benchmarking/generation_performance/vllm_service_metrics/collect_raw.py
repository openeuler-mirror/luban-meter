...
def discover_served_model_name(
    ...
    served_model_name = (
        models[0].get("id") if isinstance(models[0], Mapping) else None
    )
...
def collect_raw_result(
    ...
    if not isinstance(served_model_name, str) or not served_model_name:
        served_model_name = discover_served_model_name(
...
def main() -> None:
    ...
    result = collect_raw_result(request_data, parameters)
