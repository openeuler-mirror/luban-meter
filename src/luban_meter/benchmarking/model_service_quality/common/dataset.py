...
# ``data/ceval/val.jsonl`` falls back to
# ``<model_service_quality_dir>/data/ceval/val.jsonl``
# so the default YAML configs work from both a source checkout and a
# wheel
...
def resolve_data_path(path: str | Path) -> Path:
    ...
    verbatim. Other relative paths fall back to the bundled package
    data directory so default configs resolve stably regardless of
    the install layout.
    """
    file = Path(path)
...
def load_records(path: str | Path) -> list[dict[str, Any]]:
    ...
    if file.suffix == ".jsonl":
        ...
        # splitlines():
        # official GSM8K text contains U+2028/U+2029 which splitlines()
        # would
...
def select_records(
    ...
    *,
    ...
    """Deterministically sample records; shuffle then truncate when
    asked.
    """
