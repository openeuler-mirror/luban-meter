...
def validate_record(
    ...
    if not isinstance(input_text, str):
        raise ValueError(
            f"record {line_num} in {source}: "
            f"'input' must be string"
        )
    if not isinstance(output_text, str):
        raise ValueError(
            f"record {line_num} in {source}: "
            f"'output' must be string"
        )
...
def convert_record(
    record: dict, index: int, prefix: str
) -> dict[str, str]:
...
def validate_dataset_integrity(
    ...
    if actual_count != expected_count:
        raise ValueError(
            f"{source}: expected {expected_count} "
            f"records, got {actual_count}"
        )
...
def write_jsonl(
    records: list[dict], out_file: Path
) -> None:
...
def download_from_huggingface(
    ...
    print(
        "Downloading LCSTS test.json from "
        "Hugging Face..."
    )
    ...
    print(
        "Downloading LCSTS train.json from "
        "Hugging Face..."
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Number of few-shot examples to extract "
            "(default: 0)"
        ),
    )
    ...
    if args.source is None:
        ...
        if not test_file.is_file():
            raise FileNotFoundError(
                f"test.json not found in "
                f"{args.source}"
            )
        if not train_file.is_file():
            raise FileNotFoundError(
                f"train.json not found in "
                f"{args.source}"
            )
    ...
    if args.few_shot > 0:
        if not train_file.is_file():
            raise FileNotFoundError(
                "train.json required for few-shot "
                "but not found"
            )
        print(
            f"Extracting {args.few_shot} few-shot "
            f"examples..."
        )
        train_records = load_jsonl(train_file)
        validate_all_records(
            train_records, train_file.name
        )
    ...
    if args.few_shot > 0:
        print(
            f"  Few-shot: {args.few_shot} records"
        )
