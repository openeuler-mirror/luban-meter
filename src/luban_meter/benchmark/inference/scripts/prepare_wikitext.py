...
Usage:
    python src/luban_meter/benchmark/inference/scripts/prepare_wikitext.py \
...
def write_jsonl(
    records: list[dict[str, object]], out_file: Path
) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "directory containing wiki.valid.raw "
            "and/or wiki.test.raw"
        ),
    )
    parser.add_argument(
        "--out", type=Path, default=Path("data/wikitext")
    )
    parser.add_argument(
        "--prefix",
        type=str,
    ...
    for name, out_name in [
        ...
        for i, record in enumerate(records):
            record["id"] = f"{args.prefix}-{out_name.split('.')[0]}-{i}"
