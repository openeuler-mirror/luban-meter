"""Recompute SQuAD 2.0 EM/F1 from per-sample predictions and references."""
...
def aggregate(samples):
    ...
    return {
        key: scalar(
            sum(s[key] for s in samples) / count if count else None,
            "ratio",
            count,
        )
...
def process(raw_result: Mapping) -> dict:
    ...
    for row in rows:
        ...
        if status == "success":
            ...
            em, f1 = score_answer(prediction, answers)
        samples.append(
            {
                "exact_match": em,
                "token_f1": f1,
    ...
    metrics = {
        **aggregate(samples),
        "has_answer": aggregate(
            [s for s in samples if not s["is_impossible"]]
        ),
        "no_answer": aggregate([s for s in samples if s["is_impossible"]]),
