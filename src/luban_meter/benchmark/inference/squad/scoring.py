...
from luban_meter.benchmark.inference.common.metrics import token_f1
...
def normalize_answer(text: str) -> str:
    """Lowercase, remove ASCII punctuation, articles, then extra spaces."""
...
def parse_answer(text: str) -> str:
    """Accept an exact refusal marker; otherwise preserve answer content.

    Empty output is a parse failure, not a deliberate abstention. No substring
    matching, explanation extraction, punctuation truncation or case folding
    is applied before recognizing the marker.
    """
...
def score_answer(prediction: str, answers: list[str]) -> tuple[float, float]:
    """Return EM/F1 in [0, 1], maximizing independently over references."""
    golds = [normalize_answer(a) for a in answers if normalize_answer(a)]
    golds = golds or [""]
