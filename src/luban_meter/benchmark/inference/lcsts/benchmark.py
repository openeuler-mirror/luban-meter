...
stored for ROUGE scoring in result.py, matching
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="LCSTS inference benchmark",
    )
    parser.add_argument(
...
def validate_parameters(
    ...
    config: dict[str, Any] = {
        ...
        "few_shot_path": string_value(
            parameters, "few_shot_path", "",
        ),
        "few_shot": non_negative_integer(
            parameters, "few_shot", 0,
        ),
        "eval_mode": enum_value(
            parameters, "eval_mode",
            ("gen",), "gen",
        ),
        "prompt_format": enum_value(
            parameters, "prompt_format",
            ("chat", "base"), "chat",
        ),
    ...
    validate_prompt_version(
        "lcsts", config["prompt_version"],
    )
    if (
        config["few_shot"] > 0
        and not config["few_shot_path"]
    ):
        raise ValueError(
            "few_shot_path is required when "
            "few_shot > 0",
        )
    return config
...
def prepare_samples(
    ...
    for record in records:
        ...
        if not isinstance(abst, str) or not abst.strip():
            raise ValueError(
                "sample abst must be a non-empty string"
            )
        prepared.append({
