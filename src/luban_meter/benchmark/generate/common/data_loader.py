...
from luban_meter.benchmark.generate.common.http_client import post_json
...
def load_burstgpt_trace(
    ...
    Each record has: ``timestamp``, ``input_token``, ``output_token``.
    Returns a list of dicts with keys ``offset_s``, ``input_tokens``,
    ``output_tokens``.
    ...
    with path.open(encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            try:
                ...
                in_tok = int(row.get("input_token", 0))
                out_tok = int(row.get("output_token", 0))
    ...
    if not traces:
        raise ValueError(
            f"no valid trace records found in {dataset_path}"
        )
