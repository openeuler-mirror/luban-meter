...
def build_json_pointer(parts: list[str]) -> str:
    return "/" + "/".join(
        path_component.replace("~", "~0").replace("/", "~1")
        for path_component in parts
    )


def resolve_metric_pointer(
    value: Any,
    path: str,
    *,
    unwrap: bool = True,
...
def iter_metric_rows(
    value: Any, parts: list[str] | None = None, unit: str = ""
):
    """Yield numeric leaves, retaining nulls and omitting free-form
    text.
    """
    parts = parts or []
    if isinstance(value, Mapping):
        ...
        for key, child in sorted(value.items(), key=lambda item: str(item[0])):
            ...
            yield from iter_metric_rows(child, [*parts, str(key)], child_unit)
    elif isinstance(value, list):
        for index, child in enumerate(value):
            yield from iter_metric_rows(child, [*parts, str(index)], unit)
    elif value is None or isinstance(value, (int, float)):
        if not isinstance(value, bool):
            yield {
                "metric": build_json_pointer(parts),
...
def build_report_from_result(
    ...
    if schema == RESULT_SCHEMA:
        ...
        return ResultReport(
            ...
            payload["status"],
            source,
            [ReportTask(payload["benchmark"], payload, source)],
        )
    ...
    for entry in entries:
        ...
        if output is None:
            ...
            data, note = output, []
        tasks.append(ReportTask(task_name, data, child, note))
    return ResultReport(
        payload["suite_id"],
...
def load_report(path: Path, name: str | None = None) -> ResultReport:
    path = path.expanduser().resolve()
    if path.is_dir():
        matches = [
            candidate_path
            for candidate_path in (
                path / "suite_result.json",
                path / "result.json",
            )
        ...
        path = matches[0]
    return build_report_from_result(read_json(path), path, name)
